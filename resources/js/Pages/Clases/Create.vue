<template>
    <app-layout title="Clases - Agregar">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Instituciones / {{ institucion.nombre }} / Cursos / {{ curso.nombre }} / Materias / {{ materia.nombre }} / Clases / Agregar
            </h2>
        </template>
...
<script>
    ...
    export default defineComponent({
        ...
        methods: {
            submit() {
                this.$inertia.post(this.route('clases.store', [this.institucion.id, this.curso.id, this.materia.id]), this.form);
            },
        }
    })
</script>
